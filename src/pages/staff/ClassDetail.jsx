    ...
    return (
      <StaffDashboardLayout>
        <div className='flex justify-center items-center h-[70vh]'>
          <div className='w-10 h-10 border-4 border-orangeFpt-200 border-t-orangeFpt-600 rounded-full animate-spin'></div>
        </div>
      </StaffDashboardLayout>
    );
    ...
    <StaffDashboardLayout>
      <div className='min-h-screen space-y-8 bg-gradient-to-br from-gray-50 to-gray-100 p-6 pb-10'>
        ...
        <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
          ...
          <button
            onClick={() => navigate(-1)}
            className="mb-4 flex items-center gap-2 text-slate-600 hover:text-orangeFpt-600 transition-colors font-medium text-sm"
          >
          ...
          <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-bold ${classDetail.isActive ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-600 border-slate-200'
                  }`}>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-center gap-3 mb-2">
              <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-orangeFpt-50 text-orangeFpt-600">
              ...
              {classDetail.teams.length > 0 ? (
                <div className="flex flex-col gap-4">
                  {classDetail.teams.map((team, index) => (
                    <div key={team.teamId || index} className="group relative rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:border-orangeFpt-200 hover:shadow-md">
                      <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
                        ...
                        <div className="flex-1 space-y-3">
                          <div className="flex items-center gap-3">
                            ...
                            />
                            <h3 className="text-lg font-bold text-slate-900 group-hover:text-orangeFpt-600 transition-colors">
                              {team.teamName}
                            </h3>
                          </div>
                ...
                {classDetail.projectAssignments.length > 0 ? (
                  <div className="flex flex-col gap-4 max-h-[600px] overflow-y-auto pr-1 custom-scrollbar">
                    {classDetail.projectAssignments.map((project) => (
                      <div key={project.projectAssignmentId} className="group flex flex-col gap-3 rounded-xl border border-slate-100 bg-slate-50 p-4 transition-all hover:border-orangeFpt-200 hover:bg-orangeFpt-50/30 hover:shadow-sm">
                        <div className="flex justify-between gap-2">
                          ...
                          <div className="h-8 w-8 flex items-center justify-center rounded-lg bg-white border border-slate-100 shadow-sm">
                            <FileText className="h-4 w-4 text-orangeFpt-500" />
                          </div>
                        </div>
        ...
        <div className='space-y-4'>
          ...
          <div className='relative'>
            <input
              ...
              className='w-full pl-10 rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm text-slate-700 placeholder:text-slate-400 focus:border-orangeFpt-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-orangeFpt-100 transition-all'
            />
            <Search className="absolute left-3 top-3 h-4 w-4 text-slate-400" />
            <span className='absolute right-3 top-3 text-slate-400 text-xs'>
              {filteredStudentList.length} of {studentList.length}
            </span>
          </div>
          ...
          {filteredStudentList.length === 0 ? (
            <div className='text-center py-12 bg-slate-50 rounded-xl border border-dashed border-slate-200'>
              ...
              <p className='text-slate-500 font-medium'>No students available</p>
            </div>
          ) : (
            <>
              {/* Student List */}
              <div className='max-h-96 overflow-y-auto border border-slate-200 rounded-xl'>
                <table className='w-full text-sm'>
                  <thead className='bg-slate-50 sticky top-0 z-10 border-b border-slate-200'>
                    <tr>
                      <th className='px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase'>Select</th>
                      ...
                      <th className='px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase'>Major</th>
                    </tr>
                  </thead>
                  <tbody className='divide-y divide-slate-100'>
                    {filteredStudentList.map(s => (
                      <tr key={s.uId} className='hover:bg-slate-50 transition-colors'>
                        <td className='px-4 py-3'>
                          <input
                            type='checkbox'
                            checked={selectedStudents.some(st => st.studentId === s.uId)}
                            onChange={() => handleCheckboxChange(s.uId, s.fullname)}
                            className='w-4 h-4 text-orangeFpt-600 rounded border-slate-300 focus:ring-orangeFpt-500 cursor-pointer'
                          />
                        </td>
                        <td className='px-4 py-3'>
                          <div className='flex items-center gap-3'>
                            <Avatar
                              src={s.avatarPublicId ? `https://res.cloudinary.com/dn5xgbmqq/image/upload/v1/${s.avatarPublicId}` : null}
                              name={s.fullname}
                        ...
                        <td className='px-4 py-3'>
                          <span className='font-mono text-slate-600'>{s.studentCode}</span>
                        </td>
                        <td className='px-4 py-3'>
                          <span className='text-slate-600'>{s.major || 'N/A'}</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
              ...
              <div className='flex items-center justify-between pt-2 border-t border-slate-200'>
                <span className='text-sm text-slate-600'>
                  <span className='font-semibold text-orangeFpt-600'>{selectedStudents.length}</span> student{selectedStudents.length !== 1 ? 's' : ''} selected
                </span>
                <div className='flex gap-3'>
                  <button
                    onClick={() => setShowStudentModal(false)}
                    className='px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg transition-all font-medium text-sm'
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className='px-4 py-2 bg-orangeFpt-500 hover:bg-orangeFpt-600 disabled:bg-slate-300 disabled:cursor-not-allowed text-white font-medium rounded-lg shadow-lg shadow-orangeFpt-500/20 transition-all text-sm'
                  >
                    Add Selected
                  </button>
                </div>
      ...
      >
        <div className='space-y-4'>
          ...
          <div className='relative'>
            ...
            <Search className="absolute left-3 top-3 h-4 w-4 text-slate-400" />
            <span className='absolute right-3 top-3 text-slate-400 text-xs'>
              {filteredLecturerList.length} of {lecturerList.length}
            </span>
          </div>
          ...
          {filteredLecturerList.length === 0 ? (
            <div className='text-center py-12 bg-slate-50 rounded-xl border border-dashed border-slate-200'>
              ...
              <p className='text-slate-500 font-medium'>No lecturers available</p>
            </div>
          ) : (
            <div className='max-h-96 overflow-y-auto border border-slate-200 rounded-xl'>
              <table className='w-full text-sm'>
                <thead className='bg-slate-50 sticky top-0 z-10 border-b border-slate-200'>
                  <tr>
                    <th className='px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase'>Lecturer</th>
                    <th className='px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase'>Code</th>
                    <th className='px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase'>Major</th>
                ...
                <tbody className='divide-y divide-slate-100'>
                  {filteredLecturerList.map(l => (
                    <tr key={l.uId} className='hover:bg-slate-50 transition-colors'>
                      <td className='px-4 py-3'>
                        <div className='flex items-center gap-3'>
                          <Avatar
                            src={l.avatarPublicId ? `https://res.cloudinary.com/dn5xgbmqq/image/upload/v1/${l.avatarPublicId}` : null}
                            name={l.fullname}
                            className="w-9 h-9 rounded-full border border-slate-200"
                          />
                      ...
                      <td className='px-4 py-3'>
                        <span className='font-mono text-slate-600'>{l.lecturerCode}</span>
                      </td>
                      <td className='px-4 py-3'>
                        <span className='text-slate-600'>{l.major || 'N/A'}</span>
                      </td>
                      <td className='px-4 py-3 text-right'>
                        <button
                          onClick={() => handleAddLecturer(l.uId)}
                          className='px-4 py-1.5 bg-orangeFpt-500 hover:bg-orangeFpt-600 text-white text-xs font-semibold rounded-lg transition-all shadow-sm'
                        >
                          Assign
                        </button>
                      </td>
      ...
      >
        <UpdateClassForm
          ...
          onClose={() => {
            ...
            // Refresh class detail after update
            getClassDetail(classId).then(updatedClass => setClassDetail(updatedClass));
          }}
        />
      </ModalWrapper>
    </StaffDashboardLayout>
