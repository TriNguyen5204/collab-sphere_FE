...
const AccountCard = ({ account, accountType, onClick, index }) => {
  ...
  const code =
    accountType === 'lecturer' ? account.lecturerCode : account.studentCode;

  return (
    <button
      type='button'
      onClick={handleClick}
      className={`${glassPanelClass} flex h-full w-full flex-col rounded-3xl bg-gradient-to-bl ${getAccountGradient(
        index
