          ...
          <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
            <div className="space-y-4 max-w-2xl">
              <div>
                 ...
                 <h1 className="text-3xl font-bold text-slate-900 mt-1">Project Library</h1>
              </div>
        ...
        <div className="grid grid-cols-1 gap-8 xl:grid-cols-12">
          ...
          <div className="xl:col-span-8 2xl:col-span-9 space-y-6">
            ...
            {isLoading ? (
               ...
               <>
                 {viewMode === 'grid' ? (
                   /* --- GRID VIEW (Updated to lg:grid-cols-3) --- */
                   <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                         ...
                         <div key={project.projectId} className="group relative flex flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-all hover:border-orangeFpt-200 hover:shadow-lg">
                            ...
                            <div className="flex gap-2 mt-auto">
                               <button 
                                  ...
                                  className="flex-1 rounded-lg border border-slate-200 bg-white py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 hover:border-slate-300 transition-all"
                               >
                                  Details
                               </button>
                               <button 
                                  onClick={() => navigate(`/lecturer/projects/${project.projectId}/analysis`)}
                                  className="flex-1 rounded-lg bg-slate-900 py-2 text-xs font-semibold text-white hover:bg-orangeFpt-600 transition-all shadow-sm"
                               >
                                  Insights
                               </button>
                            </div>
                      ...
                      {filteredProjects.map((project) => (
                         <div key={project.projectId} className="group flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-all hover:border-orangeFpt-200 hover:shadow-md sm:flex-row sm:items-center">
                            ...
                            <div className="flex gap-2 sm:w-auto w-full">
                               <button 
                                  onClick={() => navigate(`/lecturer/projects/${project.projectId}`)}
                                  className="flex-1 sm:flex-none px-4 py-2 rounded-lg border border-slate-200 text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-colors"
                               >
                                  View
                               </button>
                               <button 
                                  onClick={() => navigate(`/lecturer/projects/${project.projectId}/analysis`)}
                                  className="flex-1 sm:flex-none px-3 py-2 rounded-lg bg-orangeFpt-50 text-orangeFpt-600 hover:bg-orangeFpt-100 transition-colors"
                                  title="AI Insights"
                               >
