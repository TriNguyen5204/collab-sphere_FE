    ...
    <div className="border border-slate-200 rounded-xl overflow-hidden bg-white transition-all hover:shadow-md">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border uppercase tracking-wider ${(checkpoint.statusString === 'DONE' || checkpoint.status === 1) ? 'bg-emerald-50 text-emerald-700 border-emerald-100' :
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        try {
          ...
          const currentSemester = semesterList.find(sem => sem.semesterName === teamData.semesterName);
          ...
          console.log('Fetched semester data:', currentSemester);
        } catch (semError) {
                  ...
                  <form onSubmit={handleSaveDetails} className="space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        required
                        min={semesterStartDate?.toISOString().slice(0, 10)}
                        max={formValues.endDate || semesterEndDate?.toISOString().slice(0, 10)}
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        required
                        min={formValues.startDate || semesterStartDate?.toISOString().slice(0, 10)}
                        max={semesterEndDate?.toISOString().slice(0, 10)}
                      />
                    </div>
                    <button
                      type="submit"
