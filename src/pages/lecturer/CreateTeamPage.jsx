...
const StudentListItem = ({ student, isSelected, onToggle }) => {
  ...
  return (
    ...
    >
      {/* Avatar Section */}
      <div className={`h-10 w-10 shrink-0 rounded-full flex items-center justify-center font-bold text-xs overflow-hidden border ${
        shouldShowImage ? 'border-slate-200' : `${colorClass} border-transparent`
      }`}>
        {shouldShowImage ? (
          <img 
      ...
      <div className={`w-5 h-5 rounded-full flex items-center justify-center border transition-all ${
        isSelected ? 'bg-orangeFpt-500 border-orangeFpt-500' : 'border-slate-300 bg-white'
      }`}>
    ...
    <DashboardLayout>
      ...
      <div className="min-h-screen space-y-8 bg-slate-50/50 p-6 lg:p-8">
        ...
        <form onSubmit={handleSubmit} className="mx-auto grid grid-cols-1 gap-8 lg:grid-cols-3">
          ...
          {/* RIGHT: MEMBERS */}
          <div className="space-y-6">
             <div className={`rounded-2xl border bg-white p-6 shadow-sm flex flex-col h-full max-h-[600px] transition-colors ${errors.members ? 'border-red-300 ring-2 ring-red-100' : 'border-slate-200'}`}>
