...
const HeadHeader = ({
  ...
  const { initials, colorClass, setImageError, shouldShowImage } = useAvatar(fullName, avatar);
        ...
        <div className="flex items-center gap-10 w-full">
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="leading-tight">
              <div className="text-sm font-bold text-gray-900">CollabSphere</div>
              <div className="text-xs text-orange-600 font-semibold">Head Department</div>
            </div>
          </div>
                  ...
                  {suggestions.map((suggestion, idx) => {
                    ...
                    return (
                      <li key={`${suggestion.href}-${idx}`}>
                        ...
                        >
                          {Icon ? <Icon className="w-4 h-4 text-orange-600" /> : null}
            ...
            {isAuthenticated ? (
              <>
                ...
                >
                  ...
                  <span className="hidden sm:inline max-w-[160px] truncate">{fullName || 'Head Dept'}</span>
                  <ChevronDown className="w-4 h-4 text-gray-400" />
                </button>
                {openProfile && (
                  <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden z-[80]">
    ...
    {!isAuthenticated && (
      <div className="px-4 pt-6">
        <div className="rounded-2xl border border-dashed border-gray-300 bg-gray-50 p-4 text-center">
          <p className="text-sm font-medium text-gray-700">Sign in to access tools</p>
...
const HeadDashboardLayout = ({ children }) => {
  ...
  const navigationItems = [
    {
      ...
      match: path => path === '/head-department' || path === '/head-department/'
    },
    {
    ...
    },
    {
      ...
      icon: FolderKanban,
      match: path => path.startsWith('/head-department/project-management') && !path.startsWith('/head-department/project-approvals')
    },
    {
  ...
  return (
    <div className='flex h-screen w-full bg-gray-50 overflow-hidden font-sans'>
      ...
      <div className='flex w-full pt-[64px]'> {/* Padding top to account for fixed header height approx 64px */}
        {/* Fixed Sidebar */}
