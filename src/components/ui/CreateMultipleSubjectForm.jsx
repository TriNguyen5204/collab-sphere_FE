...
import { Upload, Download, CheckCircle, AlertCircle, BookOpen, Send, X, FileSpreadsheet } from "lucide-react";
    ...
    const template = [
      {
        ...
        NoCredit: 3,
        SubjectOutcomes: "Understand basic programming concepts",
        SubjectGradeComponents: "Midterm 30%, Final 40%, Assignment 30%",
      },
    ];
        ...
        parsedData.forEach((subject, index) => {
          ...
          if (rowErrors.length > 0) {
            validationErrors.push({
              row: index + 2,
              name: subject.SubjectCode || subject.SubjectName || "Không có tên",
              errors: rowErrors,
            });
          }
        });
        ...
        <div className="p-6">
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-600 mb-3">
                  Tải xuống file Excel mẫu và điền thông tin môn học theo định dạng
                </p>
                <button
          ...
          {uploadStatus === "processing" && (
            <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg flex items-center gap-3">
              ...
              <span className="text-yellow-800 font-medium">Đang xử lý file...</span>
            </div>
          )}
                      ...
                      <div key={idx} className="text-sm">
                        {err.general ? (
                          ...
                          <p className="text-red-700">
                            <span className="font-medium">Dòng {err.row}</span> ({err.name}):{" "}
            ...
            <div className="mb-6">
                ...
                <div className="max-h-96 overflow-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-gray-100 sticky top-0">
                      <tr>
                        <th className="px-3 py-2 text-left font-semibold text-gray-700 whitespace-nowrap">STT</th>
                        <th className="px-3 py-2 text-left font-semibold text-gray-700 whitespace-nowrap">Mã môn</th>
                        <th className="px-3 py-2 text-left font-semibold text-gray-700 whitespace-nowrap">Tên môn học</th>
                        <th className="px-3 py-2 text-left font-semibold text-gray-700 whitespace-nowrap">Số tín chỉ</th>
                        <th className="px-3 py-2 text-left font-semibold text-gray-700 whitespace-nowrap">Giáo trình</th>
                        <th className="px-3 py-2 text-left font-semibold text-gray-700 whitespace-nowrap">Trạng thái</th>
                    ...
                    <tbody>
                      {subjects.map((subject, idx) => (
                        <tr key={idx} className="border-t border-gray-200 hover:bg-gray-50">
                          ...
                          <td className="px-3 py-2 font-medium text-indigo-600">{subject.SubjectCode}</td>
                          <td className="px-3 py-2 font-medium text-gray-800">{subject.SubjectName}</td>
                          <td className="px-3 py-2 text-gray-600 text-center">{subject.NoCredit}</td>
                          <td className="px-3 py-2 text-gray-600">{subject.SyllabusName}</td>
                          <td className="px-3 py-2">
                            <span className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${
                              ...
                              {subject.IsActive === true || subject.IsActive === "true" ? "Hoạt động" : "Không hoạt động"}
                            </span>
                          </td>
                          <td className="px-3 py-2 text-gray-600 max-w-xs truncate" title={subject.Description}>
                            {subject.Description}
                          </td>
                        </tr>
                      ))}
              ...
              <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <p className="text-sm text-blue-800">
                  <strong>Lưu ý:</strong> Các trường SubjectOutcomes và SubjectGradeComponents sẽ được xử lý khi gửi lên server.
          ...
          {uploadStatus === "empty" && (
            <div className="mb-6 p-4 bg-gray-50 border border-gray-200 rounded-lg text-center">
              <p className="text-gray-600">Không tìm thấy dữ liệu hợp lệ trong file</p>
            </div>
          )}
          ...
          {subjects.length > 0 && errors.length === 0 && (
            <div className="flex items-center justify-between pt-4 border-t border-gray-200">
              <p className="text-sm text-gray-600">
                Sẵn sàng tạo <span className="font-semibold text-gray-800">{subjects.length}</span> môn học
              </p>
              <button
