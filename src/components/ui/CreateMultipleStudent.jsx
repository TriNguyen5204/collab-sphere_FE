...
import { Download, Upload, Users, Check, AlertCircle, FileText, User, MapPin, Phone, Calendar, GraduationCap, Building2, BookOpen, Image } from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto p-6 bg-white">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-3">
          ...
          <h2 className="text-2xl font-bold text-gray-900">Create Multiple Student Accounts</h2>
        </div>
        <p className="text-gray-600">Upload Excel file to create multiple student accounts at once</p>
      </div>

      {/* Step indicator */}
      <div className="mb-8">
        <div className="flex items-center justify-between text-sm">
          <div className={`flex items-center gap-2 ${uploadStatus ? 'text-green-600' : 'text-green-600'}`}>
            <div className={`w-8 h-8 rounded-full flex items-center justify-center ${uploadStatus ? 'bg-green-100 text-green-600' : 'bg-green-100 text-green-600'}`}>
          ...
          <div className={`flex items-center gap-2 ${students.length > 0 ? 'text-green-600' : 'text-gray-400'}`}>
            <div className={`w-8 h-8 rounded-full flex items-center justify-center ${students.length > 0 ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 rounded-xl p-6">
          ...
          <p className="text-green-700 text-sm mb-4">
            Download the Excel template with sample student data and instructions
          </p>
          <button
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-8">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h4 className="font-medium text-amber-900 mb-2">Student Template Requirements:</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-amber-800">
              <div>• <strong>Fullname:</strong> Full student name (required)</div>
              <div>• <strong>Address:</strong> Complete address (required)</div>
              <div>• <strong>Phone:</strong> Phone number 10-11 digits (required)</div>
              <div>• <strong>Year of Birth:</strong> Birth year YYYY format (optional)</div>
              <div>• <strong>Avatar:</strong> Avatar image URL (optional)</div>
              <div>• <strong>School:</strong> School name (required)</div>
              <div>• <strong>Major:</strong> Field of study (required)</div>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50 border-b">
                <tr>
                  <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">Student Info</th>
                  <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">Contact</th>
                  <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">Academic</th>
      ...
      {isSubmitting && (
        <div className="text-center py-8">
          ...
          <p className="text-green-700">
            All {students.length} student accounts have been created successfully!
          </p>
