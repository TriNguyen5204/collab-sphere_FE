...
import { Loader2, Camera, Phone, MapPin } from "lucide-react";
...
const ProfileInformation = ({ user, avatar, onUpdateProfile, isSaving, onUploadAvatar, isUploadingAvatar, readOnly }) => {
  ...
  const handleInputChange = (event) => {
    ...
    // SHAKE LOGIC: Numbers only for phone
    if (name === "phone") {
      if (!/^[0-9]*$/.test(value)) {
        setIsShaking(true);
  ...
  // STYLES
  // Changed: Removed max-width, added w-full
    ...
    <div className={glassCard}>
      ...
      <div className="flex flex-col md:flex-row items-center gap-8 mb-10 pb-8 border-b border-gray-200/50">
        <div className="relative group">
            <div className={`w-32 h-32 rounded-full ring-4 ring-white/50 shadow-lg flex items-center justify-center overflow-hidden ${shouldShowImage ? '' : colorClass}`}>
                {shouldShowImage ? (
            ...
            {canEdit && (
                <button onClick={triggerAvatarUpload} className="absolute inset-0 bg-black/30 opacity-0 group-hover:opacity-100 rounded-full flex items-center justify-center transition-all backdrop-blur-sm">
                     {isUploadingAvatar ? <Loader2 className="animate-spin text-white" /> : <Camera className="text-white w-8 h-8" />}
                </button>
            )}
            <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={handleAvatarChange} />
        </div>
        <div className="text-center md:text-left">
            <h2 className="text-3xl font-bold text-gray-800 mb-1">{profileData.fullname}</h2>
            <div className="flex flex-wrap gap-2 justify-center md:justify-start">
                <span className="px-3 py-1 bg-white/50 rounded-lg text-sm font-medium text-gray-600 border border-white/40 shadow-sm">{profileData.code}</span>
                <span className="px-3 py-1 bg-orangeFpt-50 rounded-lg text-sm font-bold text-orangeFpt-600 border border-orangeFpt-100 shadow-sm">
      ...
      {/* Changed: Grid expands to 3 columns on large screens to fill space */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {/* ADDRESS - Spans 2 cols on large screen */}
